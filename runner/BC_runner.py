...
# obs = open(os.path.join(dir,"obs.npy"),'rb').read()
# actions = open(os.path.join(dir,"actions.npy"),"rb").read()
class Runner_BC:
    def __init__(self,env_cfg:env_cfg,alg_cfg:alg_cfg,device='cpu',log_dir=None):
        ...
        self.dataset = {
            "obs": np.load('runner/obs.npy'),
            "actions": np.load('runner/actions.npy'),
            "next_obs": np.load('runner/next_obs.npy'),
            "rewards": np.load('runner/rewards.npy'),
            "terminals": np.load('runner/dones.npy')
        }
        self.max_size = self.dataset["obs"].shape[0]
        # self.env_cfg.clip_action = float(self.env.action_space.high[0])
        print("###########################",self.dataset["obs"].shape[0])
        ...
        self.data_.observation = self.dataset["obs"]
        self.data_.next_observation = self.dataset["next_obs"]
        ...
        self.actor = MLP(env_cfg.num_obs, env_cfg.num_actions).to(self.device)
        ...
        self.alg = BC(self.actor, alg_cfg.num_epochs, alg_cfg.num_mini_batches, self.data_, self.replaybuffer)
        self.writer = None
    ...
    def log(self, locs, width=80, pad=35):
        ...
        self.writer.add_scalar('Loss/actor', locs['actor_loss'], locs['it']+locs['epoch']*locs['num_learning_iterations'])
    ...
    def save(self, path, it, infos=None):
        torch.save({
            ...
            'optimizer_actor_state_dict': self.alg.optimizer.state_dict(),
